...
description: "From the perspective of circuit complexity, this paper provides a unified explanation of why constant-depth Transformers cannot exactly perform integer multiplication of arbitrary length, and why stronger linear attention variants often fail to maintain full token parallelism."
...
> This post is based on a note I wrote half a year ago, titled "When we talk about LLM reasoning, what are we talking about?" At that time, I had just joined Hillhouse and was working on LLM latent reasoning (although now I think neither CoT nor latent reasoning touches the essence). Recently, someone revisited that note, and I found some parts outdated, so I reorganized it and published it as a blog post.
...
In practice, a recurring phenomenon is: architectures that are easiest to scan have the weakest expressiveness; while variants that introduce more complex gates, stronger state updates, or state transitions that explicitly depend on the current hidden state, although indeed more expressive, often can only achieve chunk-wise parallelism, or even degrade to parallel training but sequential inference.
...
- $$\mathsf{TC}^0$$: allows threshold (majority) gates on top of $$\mathsf{AC}^0$$, thus can perform global counting and threshold judgments that pure AND/OR cannot.
...
These two upper bounds are not contradictory because they correspond to different model settings. When discussing the theoretical expressive power of Transformers, one must clearly distinguish:
...
## 6. Expressive power upper bounds for linear recurrent models
...
But this parallelism advantage also constitutes a constraint on expressive power. Related theoretical results [3] show that the expressive power upper bound of such basic linear state space models also falls within $$\mathsf{TC}^0$$. Although these models have a recurrent form, because the recurrence is regular enough that the entire computation can be fully parallelized into a scan, the computational graph is essentially shallow.
...
Any of these modifications may break the associative summary structure required by scan. Once it is impossible to accurately summarize the effect of an arbitrarily long prefix on the suffix using a fixed-size summary and an associative operator, the model can only fall back to one of the following schemes: unfold sequentially along the sequence; parallelize within chunks but sequential between chunks; or replace the exact scan with an approximate scan.
...
> The core observation is: to significantly surpass the expressiveness upper bound of shallow parallel architectures, the effective depth must grow with the input length; and the growth of effective depth inevitably, in turn, weakens token parallelism.


## 9. Formal Statement

Write the above trade-off as a formal proposition. Let $$\mathfrak A(g)$$ be the subclass of models in architecture family $$\mathfrak A$$ that all satisfy $$D_{\mathfrak A}(n) = O(g(n))$$. For a target function $$L$$, define

$$
d_{\mathfrak A,L}(n) := \inf \left\{ g(n) \;\middle|\; \exists M \in \mathfrak A(g),\ M \text{ 能计算 } L \right\}.
$$

$$d_{\mathfrak A,L}(n)$$ characterizes the minimum effective depth required to express $$L$$ within the architecture family $$\mathfrak A$$. From this, the following corollaries follow directly from the definitions of complexity classes and known separation results:

- If $$L \notin \mathsf{TC}^0$$, then any architecture family that is $$D(n) = O(1)$$ and can be simulated by uniform constant-depth threshold circuits cannot express $$L$$.
- If $$L \notin \mathsf{AC}^0$$, then any architecture family simulated by uniform $$\mathsf{AC}^0$$ circuits cannot express $$L$$.
...
The root of this impossibility lies in the fact that both the depth of the computational graph and the numerical state space are constant, unable to carry the global carry propagation that grows with $$n$$. CoT expands the effective depth to $$O(t(n))$$ by introducing intermediate generation steps, allowing the carry to be propagated step by step.

### 10.2 Problem 2

Full token parallelism relies on the feasibility of prefix scan, which requires the state update to satisfy a strongly associative structure. Basic linear recurrent models that satisfy this structure are limited to $$\mathsf{TC}^0$$. The modifications needed to enhance expressiveness—more general input dependence, explicit dependence on the previous hidden state, and gates that do not satisfy associativity—precisely destroy the scan structure, causing the model to degenerate into chunk-wise parallelism or sequential computation.
...
The core argument of this paper is that stronger sequential expressiveness must come at the cost of increased effective depth.

Fixed-depth Transformers and basic linear recurrent models that can be fully scanned belong to the same extremely shallow parallel computation class, with an expressiveness upper bound of $$\mathsf{TC}^0$$ (tightened to $$\mathsf{AC}^0$$ under constant precision stepwise rounding semantics). Tasks such as arbitrary $$n$$-bit integer multiplication, general regular language recognition, and complex state tracking require handling dependency chains that grow with $$n$$, which are beyond these upper bounds. CoT, looping, and stronger state-dependent recurrence can break these upper bounds, uniformly because they introduce effective computational depth that grows with $$n$$; this growth inevitably weakens the strongest form of token parallelism.

> The answers to the two problems are thus unified: constant-precision fixed-depth Transformers cannot exactly perform arbitrary-length multiplication, and stronger linear attention cannot maintain full parallelism—the root cause is the same, namely the parallel computation-expressiveness trade-off.
