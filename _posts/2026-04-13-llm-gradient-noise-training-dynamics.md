...
结果显示：SGD 的噪声协方差迹随 $$\sigma^2$$ 增长（斜率约 $$2.00$$），归一化更新协方差迹近似常数（斜率约 $$0.00$$）；同时归一化漂移项随 $$1/\sigma$$ 衰减（斜率约 $$-0.99$$），与第 3 节推导一致。
...
综合来看，第 7 节从方向一致性、漂移与协方差缩放、最大稳定步长、稳态误差地板以及异方差加权五个角度，逐项验证了本文理论结论。
