...
很多线性注意力或线性记忆结构，都可以被看成在维护一个 Key 到 Value 的线性映射矩阵 $S_t$。Gated DeltaNet [1] 的特别之处在于：它不直接写入 $K_t^\top V_t$，而是写入预测残差，并用门控控制遗忘与写入强度。
...
而 Longhorn 风格的自归一化步长在代数上正是同一族，只需令 $\varepsilon_t = 1/\gamma_t$。这表明 Longhorn 本质上属于软投影或阻尼 Kaczmarz 的一类变体。
