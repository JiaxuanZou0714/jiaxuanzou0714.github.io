...
> 本文整理自半年前我的一篇笔记。最近有人重新翻出，其中部分内容已过时，遂重新梳理后发出。
...
这两个问题指向同一个核心矛盾：**在序列建模中，最强 token 并行性与更强顺序表达能力之间是否存在结构性的 trade-off？**
...
这几个类的层级恰好对应并行计算能力的分层。$$\mathsf{AC}^0$$ 和 $$\mathsf{TC}^0$$ 代表极强的常数深度并行；$$\mathsf{NC}^1$$ 开始允许随输入长度对数增长的依赖链，能表达一般正则语言等状态追踪问题。因此，若某个架构族的所有计算都可被常数深度电路模拟，它就无法表达真正需要随 $$n$$ 增长的顺序依赖的问题。
...
$$
\text{constant-depth} + \text{constant-width} + \text{constant-bit precision} + \text{immediate rounding} \subseteq \mathsf{AC}^0.
$$
...
核心观察是：**要显著超越浅层并行架构的表达能力上界，有效深度必须随输入长度增长；而有效深度的增长不可避免地削弱最强意义下的 token 并行性。**
...
这即并行性—表达能力 trade-off 的严格含义：它不是经验规律，而是浅层电路与顺序计算之间复杂度分界的直接推论。
...
本文讨论的并行性是电路复杂度意义下的计算图深度，而非 GPU 上的 wall-clock time。理论上从 $$\mathsf{TC}^0$$ 放宽到 $$\mathsf{NC}^1$$ 并不意味着实际硬件上有同等程度的吞吐下降。现实硬件本身不直接实现 majority gate，很多 $$\mathsf{TC}^0$$ 内的操作在硬件上也需要 $$O(\log n)$$ 深度的 reduction 完成。

因此从常数深度并行放宽到对数深度并行，在工程上未必不可接受。这也使得 log-depth looping [4]、更强状态更新、chunk-wise parallel 等方向具有实际研究价值：它们放松了理论上最严格的并行性约束，但可能以较小的吞吐代价换取显著的表达能力提升。
...
两个问题的答案由此统一：常数精度固定深度 Transformer 不能精确做任意长度乘法，更强的 linear attention 不能保持完全并行——根源是同一个，即浅层并行计算与顺序表达能力之间存在由电路复杂度层级决定的结构性边界。
